...
namespace DeliveryTracking.Application.Features.Orders.Commands.AcceptOrder
{
    public class AcceptOrderCommandHandler : IRequestHandler<AcceptOrderCommand, Result<OrderDto>>
    {
        ...
        public AcceptOrderCommandHandler(IUnitOfWork unitOfWork, ICurrentUserService currentUserService, ILogger<AcceptOrderCommandHandler> logger)
        {
        ...
        public async Task<Result<OrderDto>> Handle(AcceptOrderCommand request, CancellationToken cancellationToken)
        {
            ...
            if (order.Status != OrderStatus.Assigned)
            {
                return Result<OrderDto>.FailureResult(
                    ...
                    new List<string> { "Only assigned orders can be accepted." }
                );
            }
            ...
            _logger.LogInformation("Order {OrderId} accepted by Rider {RiderId}. OrderNumber: {OrderNumber}", 
                request.OrderId, _currentUserService.UserId, order.OrderNumber);

            // Map to DTO and return
            var orderDto = MapToOrderDto(order);
            return Result<OrderDto>.SuccessResult(orderDto, "Order accepted successfully");
        }
